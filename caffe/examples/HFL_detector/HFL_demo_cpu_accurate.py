...
deploy_path=caffe_root + 'examples/VGG/HFL_deploy_accurate.prototxt'
...
if '.jpg' in input_img_file or '.png' in input_img_file:
    ...
    init_file=name+'_init_acc.jpg'
    #init_file='init/init_bsds_test_nms/'+name+'.jpg'
    ...
    feat_mat= np.zeros((num_candidates,num_feats))
    ...
    #for ii in range(500):
      feat_counter=0
      orig_cords[ii,0]=list_rows[ii]
      orig_cords[ii,1]=list_cols[ii]
    ...
    # INPUT NORMALIZATION
    feat_mat=np.subtract(feat_mat,mu)
    feat_mat=np.divide(feat_mat,sigma)



    W1=hfl_data['W1']
    W1 = np.array(W1, order="C")

    W2=hfl_data['W2']
    W3=hfl_data['W3']


    feat_mat=np.c_[np.ones(num_candidates),feat_mat] #adding bias
    feat_mat = np.array(feat_mat, order="C")
   
    a1=np.dot(feat_mat,W1.T)



    a1=1.7159*np.tanh(2*a1/3)
